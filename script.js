...
document.getElementById('submit-btn').onclick = () => {
    ...
    refWords.forEach((ref, i) => {
        ...
        // Instruction 1 & 2: Special Colon & Matters/Capital Handling (Auto Green)
        if (typed === ref || typed.toLowerCase() === ref.toLowerCase() || (ref.endsWith(':') && typed === ref.replace(':',''))) {
            html += `<span class="text-correct">${ref}</span> `;
        } else if (typed === "") {
            mistakes++; html += `<span class="text-missing">${ref}</span> `; // Underline Green
        } else {
            mistakes++; html += `<span class="text-error">${typed}</span> `; // Red
        }
    });
    ...
    const wpm = Math.round((typedWords.length/5)/((900-timeLeft)/60));
